...
class OptSolution(object):
    ...
    def display(self,altruists=list()):
        ...
        for c in self.chains:
            index = c.ndd_index
            if (PRINT):
                print((str(altruists[index]) + "\t" + "\t".join(str(v) for v in c.vtx_indices)))
            chains[i] = [v for v in c.vtx_indices]
            i += 1

...
def solve_KEP(G,K,L=0,altruistic_list=[], weights={}):
    ...
    # create variables for chains: obj is
    Z = {(i,j,l): m.addVar(obj = weights[(i,j)], vtype="B",name="z"+str(i)+"_"+str(j)+"_"+str(l)) for i,j in K_dic.keys() for l in K_dic[(i,j)]}
    m.update()
    for v in G.keys():
    ...
    for i,c in enumerate(Cycles_k):
        if X[i+1].x>0.5: # that is, X[i+1]= 1; OBS: I use this 0.5 because it might happen that python sees X[i+1]=1.0 different from 1
            print(" cycle ", c , "   weight ", cweights[i])
    ...
    for v in altruistic_list:
        ...
        if aux_chain !=[]:
            chain_weight = 0
            for edge in aux_chain:
                chain_weight = chain_weight + weights[edge]
            print(" chain ", aux_chain, "  weight ", chain_weight)
            chains.append(aux_chain)
    toc1 = time.time()
    print("\n OPT = ", m.ObjVal, "  time = ", m.Runtime)
    print(" time to setup ip = %s " % str(toc3 - tic3))
    print(" time of get_all_cycles = %s " % str(toc2 - tic2))
    print(" time searching cycles = %s " % str(toc - tic))
    print(" time searching chains = %s " % str(toc1 - tic1))
